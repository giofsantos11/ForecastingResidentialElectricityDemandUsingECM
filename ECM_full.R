#####################################################################
###Performing the ECM for full data and within sample simulations
#By: ANGELO SANTOS
#Date: May 29, 2021
#####################################################################
...
##Perform one-stage ECM
model.ecm <- lm(Dln_cons ~ Dln_hhfe+ LDln_price + Dln_temp + Lln_cons + Lln_hhfe)
summary(model.ecm)
...
yhat.ecm = predict(model.ecm, type = "response")
pred.ecm <- predict(model.ecm, interval = "confidence")

##Create dataframe for actual and predict HH cons
df.cons <- data.frame(Dln_cons[2:22], yhat.ecm, HHElec$year[3:23], 
              Dln_hhfe[2:22], LDln_price[2:22], Dln_temp[2:22], Lln_cons[2:22],
              Lln_hhfe[2:22], pred.ecm)

##Rename the variables in the created data frame
colnames(df.cons) <- c("Dln_cons", "yhat.ecm", "year", "Dln_hhfe", "LDln_price",
...
df.full$ln.cons.sim<- df.full$yhat.ecm + df.full$Lln_cons
