...
model.ardl.train <- lm(Dln_cons ~ Dln_hhfe+ LDln_price + Dln_temp,
                      data = df1[train,])

summary(model.ardl.train)

##Predict point estimates and intervals
pred.ardl.test<- predict(model.ardl.train, df1[-train,], interval = "confidence")

##Merge predicted values with training data 
y <- data.frame(fit=rep(NA,18), lwr = rep(NA,18),upr = rep(NA,18))
x<- data.frame(pred.ardl.test)
z <- rbind(y,x)
